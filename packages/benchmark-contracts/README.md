...
Version 0.5.1 corrects the OpenAPI contract description, which still named 0.4.0 and its fields. No schema, type, validation or wire behaviour changes, so a 0.5.0 archive and a 0.5.1 archive accept and reject exactly the same submissions. It exists as its own version because the packed archive bytes differ and both repositories pin an archive by integrity.

Version 0.6.0 adds optional `environment.cpu.physical_cores`: the number of physical cores the measuring machine reported, beside the existing `logical_cores` thread count. The two are different numbers on any processor with simultaneous multithreading, and thread-count settings are measured against `logical_cores`, so both are published rather than one standing in for the other. A machine whose operating system does not report the count sends null; the value is never derived by dividing the thread count, because that is wrong on every processor without multithreading. As with earlier optional fields, deploy the accepting service before enabling uploads, because older validators reject fields they do not recognize. An absent value on older records stays absent and must never be filled in from the viewer's machine.

...
export default function RuntimeBackendList({
    ...
    return (
      <section key={row.backend} className="min-w-0 rounded-xl border border-slate-700 app-bg-muted p-4" aria-labelledby={`runtime-${row.backend}`} aria-busy={row.busy}>
        ...
        <div className={`runtime-latest-slot mt-2 text-[11px] ${info ? "text-slate-500" : "text-red-400"}`}>
          {info ? <span className="block truncate">{t("ui.latestBuild")}: <span className="text-slate-300">{t("ui.buildLabel", { build: buildNumber(info.build) })}</span>{" · "}{info.digest ? t("ui.digestPublished") : t("ui.digestUnavailable")}</span> : <span className="block truncate">{t("ui.latestUnavailable")}{row.latestErr ? `: ${row.latestErr}` : ` ${t("ui.latestUnavailableRetry")}`}</span>}
        </div>
        ...
        {row.busy && row.progress && <div className="runtime-progress-slot mt-3" role="progressbar" aria-label={t("ui.installedBuilds", { label: row.backend })} aria-valuemin={0} aria-valuemax={100} aria-valuenow={row.progress.total > 0 ? Math.round(row.progress.received / row.progress.total * 100) : undefined}>
          <div className="mb-1 flex justify-between gap-2 text-xs text-slate-400"><span>{t(`ui.${buildPhaseLabelKey(row.progress.phase)}`)}</span>{row.progress.total > 0 && <span>{(row.progress.received / 1048576).toFixed(1)} / {(row.progress.total / 1048576).toFixed(1)} MB</span>}</div>
        ...
        {row.installed.length > 0 && <div className="mt-3 flex min-w-0 flex-wrap gap-2.5" role="list" aria-label={t("ui.installedBuilds", { label: backendName })}>
          {row.installed.map((item) => {
            ...
            return <div key={item.build} role="listitem" className={`flex min-w-0 max-w-full flex-wrap items-center gap-2 rounded-lg border px-3 py-1.5 text-xs ${isActive ? "app-border-success bg-emerald-950/40" : "border-slate-700 app-bg-muted"}`} title={normalizeDisplayPath(item.dir)}>
              ...
              {isActive ? <span className="rounded bg-emerald-800 px-1.5 py-0.5 text-[10px] text-emerald-200">{t("ui.active")}</span> : <>
  ...
  return <div className="runtime-list space-y-3.5">
    ...
    <section className="space-y-2" aria-labelledby="runtime-available-heading"><h2 id="runtime-available-heading" className="app-section-title">{t("ui.runtimeAvailableHeading")}</h2>{availableRows.length > 0 ? availableRows.map(renderRow) : <p className="text-xs text-slate-500">{t("ui.none")}</p>}</section>
  </div>;
}

    ...
    <div className="app-page-scroll developer-panel relative flex h-full min-h-0 flex-col overflow-auto p-4" tabIndex={0} data-developer-section={section}>
      ...
      {section === "api" && <section className="developer-section developer-section--connection grid items-start gap-3 app-balanced-columns">
        <div className="rounded-xl border p-4 ui-border-color-border ui-background-panel" tabIndex={0}>
          ...
          <details className="developer-runtime-details mt-3"><summary>{t("section.diagnostics")}</summary><div className="developer-runtime-details-content flex flex-wrap gap-1.5 text-xs ui-color-faint"><span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >state: {store.status.state}</span>{store.status.pid && <span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >PID: {store.status.pid}</span>}{store.status.model && <span className="max-w-full app-text-wrap rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted"  title={normalizeDisplayPath(store.status.model)}>effective: {normalizeDisplayPath(store.status.model).split(/[\\/]/).pop()}</span>}<span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >{t("ui.authLocalBearer")}</span></div></details>
        </div>

...
export default function ChatComposer({
  ...
  return (
    <>
      ...
      {attachments.length > 0 && <div className="mt-3 flex flex-wrap gap-2" role="group" aria-label={ct("pendingImages")}>{attachments.map((image) => <div key={image.dataUrl} className="relative"><img src={image.dataUrl} alt={normalizeDisplayText(image.name)} width={64} height={64} className="h-16 w-16 rounded-lg border object-cover ui-border-color-border"  /><button type="button" onClick={() => onRemoveAttachment(image.dataUrl)} className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full border text-white ui-background-danger-solid ui-border-color-surface"  aria-label={`${ct("removeAttachment")}: ${normalizeDisplayText(image.name)}`}><svg width="8" height="8" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M3 3 9 9M9 3 3 9" /></svg></button></div>)}</div>}

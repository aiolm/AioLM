...
export default function DraftGpuEditor({ placement, device, disabled, onChange, onInvalid }: {
  ...
  return <section className="model-settings-fields" aria-label={copy.gpu}>
    ...
    <div className="model-settings-gpus">{devices.map((item, index) => <label key={item.stable_id} className="model-settings-gpu">
      <input type="checkbox" checked={gpu.gpu_ids.includes(item.stable_id)} disabled={disabled} onChange={() => { setWeights(null); onInvalid('gpu_tensor_split', false); onChange(toggleGpuSelection(gpu, item.stable_id, devices)); }} />
    ...
    <div className="model-settings-grid">
      <label>{t('ui.gpuMain')}<CustomSelect ariaLabel={t('ui.gpuMain')} value={gpu.main_gpu ?? ''} options={options.filter(option => !option.value || gpu.gpu_ids.includes(option.value))} onChange={value => patch({ main_gpu: value || null })} disabled={disabled} /></label>
      <label>{t('ui.gpuDraft')}<CustomSelect ariaLabel={t('ui.gpuDraft')} value={gpu.draft_gpu_id ?? ''} options={options} onChange={value => patch({ draft_gpu_id: value || null })} disabled={disabled} /></label>
      <label>{t('ui.gpuSplitMode')}<CustomSelect ariaLabel={t('ui.gpuSplitMode')} value={gpu.split_mode} options={['none', 'single', 'layer', 'row', 'tensor'].map(value => ({ value, label: value === 'none' ? copy.automatic : value }))} onChange={value => patch({ split_mode: value as SplitMode })} disabled={disabled} /></label>
      <label>{copy.tensorSplit}<input className="app-input" value={weights ?? gpu.tensor_split.join(', ')} disabled={disabled || gpu.gpu_ids.length < 2} onChange={event => {

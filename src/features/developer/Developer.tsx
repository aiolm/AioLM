...
export default function DeveloperPanel({ store, section = "api" }: { store: AppStore; section?: "api" | "gateways" | "diagnostics" }) {
  ...
  return (
    <div className="app-page-scroll developer-panel relative flex h-full min-h-0 flex-col" tabIndex={0} data-developer-section={section}>
      ...
      {section === "api" && <section className="developer-section developer-section--connection grid items-start gap-3 app-balanced-columns">
        <div className="app-card" tabIndex={0}>
          ...
          <details className="developer-runtime-details mt-3"><summary>{t("section.diagnostics")}</summary><div className="developer-runtime-details-content flex flex-wrap gap-1.5 text-xs ui-color-faint"><span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >state: {store.status.state}</span>{store.status.pid && <span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >PID: {store.status.pid}</span>}{store.status.model && <span className="max-w-full app-text-wrap rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted"  title={normalizeDisplayPath(store.status.model)}>effective: <ModelIcon model={store.status.model} />{modelDisplayName(store.status.model)}</span>}<span className="rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted" >{t("ui.authLocalBearer")}</span></div></details>
        </div>
        <div className="developer-loaded-models app-card" tabIndex={0}><h3 className="app-section-title">{t("ui.loadedModels")}</h3><p className="app-section-hint">{t("ui.loadedModelsHint")}</p><div className="developer-loaded-models-list"><div className="developer-model-status text-xs ui-color-faint">{!serverReady ? t("ui.serverOffline") : models.length === 0 && !loadingModels ? t("ui.noModelResponse") : ""}</div>{models.map((model) => { const displayModelId = normalizeDisplayPath(model.id); return <div key={model.id} className="mt-3 rounded-lg border p-2.5 ui-border-color-border ui-background-mono-bg" ><div className="app-text-wrap font-mono text-xs ui-color-ink"  title={displayModelId}><ModelIcon model={model.id} />{displayModelId}</div><ModelBadges model={model.id} /><div className="mt-1 text-xs ui-color-faint" >{model.owned_by || "llama.cpp"}</div></div>; })}</div></div>
      </section>}

      {section === "api" && <section className="developer-section developer-section--endpoints mt-4 app-card" ><div className="flex items-start justify-between gap-3"><div><h3 className="app-section-title">{t("ui.endpointsTitle")}</h3><p className="app-section-hint">{t("ui.endpointsHint")}</p></div><span className="rounded-full border px-2.5 py-1 text-xs tabular-nums ui-border-color-border ui-background-surface-muted ui-color-faint" >/v1</span></div><div className="mt-3.5 grid gap-2.5 app-form-grid">{ENDPOINTS.map((endpoint) => { const id = `${endpoint.method}-${endpoint.path}`; const snippet = buildCurlSnippet(displayUrl, endpoint.path); return <div key={id} className="rounded-lg border p-3 ui-border-color-border ui-background-mono-bg" ><div className="flex items-center gap-2"><span className="rounded px-1.5 py-0.5 font-mono text-xs font-semibold tracking-wide ui-background-accent-soft ui-color-accent" >{endpoint.method}</span><code className="font-mono text-xs ui-color-ink" >/v1{endpoint.path}</code></div><p className="mt-2 text-xs leading-relaxed ui-color-muted" >{t(`ui.${endpoint.description}`)}</p><button type="button" onClick={() => void copy(id, snippet)} aria-label={`${t("ui.copyCurl")}: ${endpoint.method} /v1${endpoint.path}`} className="app-button app-button--secondary app-button--sm mt-2.5"><StableLabel value={copied === id ? t("ui.copiedCurl") : t("ui.copyCurl")} labels={[t("ui.copiedCurl"), t("ui.copyCurl")]} /></button></div>; })}</div></section>}
      ...
      {section === "gateways" && <section className="developer-section developer-section--gateway mt-4 app-card" ><div className="developer-section-header flex min-w-0 flex-wrap items-start justify-between gap-3"><div className="developer-section-copy min-w-0"><h3 className="app-section-title">{t("ui.gatewayTitle")}</h3><p className="app-section-hint max-w-2xl">{t("ui.gatewayHint")}</p></div><button type="button" onClick={() => void toggleGateway()} disabled={!serverReady} className="developer-section-action app-button app-button--primary app-button--sm"><StableLabel value={gateway.running ? t("ui.stopGateway") : t("ui.startGateway")} labels={[t("ui.stopGateway"), t("ui.startGateway")]} /></button></div>{gateway.running && <div className="developer-section-status mt-3 flex min-w-0 flex-wrap items-center gap-2 text-xs ui-color-muted" ><span className="rounded-full border px-2 py-1 text-xs font-medium ui-border-color-success-border ui-background-success-bg ui-color-success-ink" >{t("ui.running")}</span><code className="developer-code-block min-w-0 break-all font-mono text-xs ui-color-accent" >{gateway.url}</code></div>}<div className="mt-2 text-xs ui-color-faint" >{t("ui.gatewayBindHint")}</div></section>}

      {section === "gateways" && <section className="developer-section developer-section--responses mt-4 app-card" ><div className="developer-section-header flex min-w-0 flex-wrap items-start justify-between gap-3"><div className="developer-section-copy min-w-0"><h3 className="app-section-title">{t("ui.responsesTitle")}</h3><p className="app-section-hint max-w-2xl">{t("ui.responsesHint")}</p></div><span className="developer-section-status rounded-full border px-2.5 py-1 text-xs ui-border-color-border ui-background-surface-muted ui-color-faint" >{gateway.running ? t("ui.responsesAvailable") : t("ui.responsesStartGateway")}</span></div><div className="developer-responses-grid mt-3.5 grid min-w-0 gap-3 app-form-grid"><code className="developer-code-block min-w-0 break-all rounded-lg p-3 font-mono text-xs ui-background-mono-bg ui-color-accent" >{gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"}</code><code className="developer-code-block min-w-0 break-all rounded-lg p-3 font-mono text-xs ui-background-mono-bg ui-color-accent" >GET/DELETE /v1/responses/&lt;id&gt;</code></div><button type="button" onClick={() => void copy("responses", `curl ${gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"} \\\n  -H "Authorization: Bearer ***" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"<MODEL_ID>","input":"Hello","previous_response_id":null,"stream":false}'`)} disabled={!gateway.running} aria-label={`${t("ui.copyResponsesCurl")}: Responses`} className="app-button app-button--secondary app-button--sm mt-3"><StableLabel value={copied === "responses" ? t("ui.copiedCurl") : t("ui.copyResponsesCurl")} labels={[t("ui.copiedCurl"), t("ui.copyResponsesCurl")]} /></button></section>}

      {section === "api" && <section className="developer-section developer-section--snippets mt-4 grid gap-3 app-form-grid"><div className="app-card" tabIndex={0}><div className="flex items-center justify-between gap-3"><h3 className="app-section-title">Python</h3><button type="button" onClick={() => void copy("python", pythonSnippet)} aria-label={`${t("ui.copy")}: Python snippet`} className="app-button app-button--secondary app-button--sm"><StableLabel value={copied === "python" ? t("panel.copied") : t("ui.copy")} labels={[t("panel.copied"), t("ui.copy")]} /></button></div><pre tabIndex={0} aria-label="Python code snippet" className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg p-3 font-mono text-xs leading-relaxed ui-background-mono-bg ui-color-mono-ink" >{pythonSnippet}</pre></div><div className="app-card" tabIndex={0}><div className="flex items-center justify-between gap-3"><h3 className="app-section-title">JavaScript</h3><button type="button" onClick={() => void copy("javascript", jsSnippet)} aria-label={`${t("ui.copy")}: JavaScript snippet`} className="app-button app-button--secondary app-button--sm"><StableLabel value={copied === "javascript" ? t("panel.copied") : t("ui.copy")} labels={[t("panel.copied"), t("ui.copy")]} /></button></div><pre tabIndex={0} aria-label="JavaScript code snippet" className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg p-3 font-mono text-xs leading-relaxed ui-background-mono-bg ui-color-mono-ink" >{jsSnippet}</pre></div></section>}

      {section === "diagnostics" && <section className="developer-section developer-section--diagnostics mt-4 app-card" ><h3 className="app-section-title">{t("ui.diagnosticsTitle")}</h3><pre tabIndex={0} aria-label={t("ui.diagnosticsTitle")} className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-lg p-3 font-mono text-xs leading-relaxed ui-background-mono-bg ui-color-mono-ink" >{normalizeDisplayText(store.status.log_tail || store.status.error || t("ui.noDiagnostics"))}</pre></section>}
